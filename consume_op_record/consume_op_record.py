...
if __name__ == "__main__":
    ...
    op_record_info_path = os.path.join(os.path.dirname(__file__), 'op_record.json')

    consumer = GeneralKafkaConsumer(topic_name, bootstrap_servers)
    writer = JSONWriter(op_record_info_path)

    try:
        for message in consumer.consume_messages():
            message_value = message.value
            if message_value.get('source') == 'pre_op_record':
                patient_details = {"op_details": message_value['value']}
