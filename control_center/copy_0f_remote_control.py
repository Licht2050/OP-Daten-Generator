...
class ScriptExecutor:
    ...
    def start_cpp_script(self, script_path , executable_name, log_path):
        make_command = f"cd {os.path.dirname(script_path)} && make"
        self.ssh.exec_command(make_command)
        command = f'./{executable_name} > {log_path} 2>&1 &'
        self.ssh.exec_command(command)
        # command = f"pgrep -f 'python {generator_script_path}'"
        command = f"ps aux | grep 'g++ {executable_name}' | grep -v grep"
        _, stdout, _ = self.ssh.exec_command(command)
...
def execute_scripts(config_loader):
    ...
    try:
        ...
        if user_input == 'j':
            ...
            pi_info["pi_name"] = pi_name
            script_executor = ScriptExecutor(pi_info)
            script_path = f"synchronization_pi"
