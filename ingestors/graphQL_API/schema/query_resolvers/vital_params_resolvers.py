...
def resolve_get_heart_rate_by_date(root, info, patientId, timestamp, secondsRange):
    ...
    requested_vital_params = get_requested_subfields(info, "getHeartRateByDate")
...
def get_requested_subfields(info, main_field_name):
        ...
        if node.name.value == main_field_name:
            ...
            if node.selection_set:
                ...
                for selection in node.selection_set.selections:
                    if selection.name.value == "vitalParams" and selection.selection_set:
